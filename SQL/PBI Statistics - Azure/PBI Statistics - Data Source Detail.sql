...
SELECT 
  ...
  CASE
    ...
    WHEN dsd.ConnectionDetails LIKE '%:"Snowflake"%' THEN
      CASE
        WHEN CHARINDEX(';', JSON_VALUE(dsd.ConnectionDetails, '$.extensionDataSourcePath')) > 0 THEN
          LEFT(JSON_VALUE(dsd.ConnectionDetails, '$.extensionDataSourcePath'), CHARINDEX(';', JSON_VALUE(dsd.ConnectionDetails, '$.extensionDataSourcePath')) - 1)
        ELSE JSON_VALUE(dsd.ConnectionDetails, '$.extensionDataSourcePath')
      END
  ...
  CASE
    ...
    WHEN dsd.ConnectionDetails LIKE '{"extensionDataSourceKind":"Snowflake",%' THEN
      CASE
        WHEN CHARINDEX(';', REVERSE(JSON_VALUE(dsd.ConnectionDetails, '$.extensionDataSourcePath'))) > 0 THEN
          RIGHT(
...
ORDER BY dsd.DatasourceID ASC, DatasourceBase ASC, DatasourceType ASC