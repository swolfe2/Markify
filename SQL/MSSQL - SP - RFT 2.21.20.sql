...
Added logic to determine the first failure, along with the count of times a load was manually touched, and the total process steps a load failed on
-- Description:	Update USCTTDEV.dbo.tblRFTDetailDataHistorical and USCTTDEV.dbo.tblRFTManuallyTouchedDetails with RFT information, when something is manually touched
-- =============================================
*/

ALTER PROCEDURE [dbo].[sp_RFT]
...
2) Aggregate line item detail into Load level detail
3) Get load leg details from Oracle
4) Get data from the Lights Out Planned process
5) Get data from the Lights Out tendered process
6) Get the highest weighted BU for each delivery, with in-memory table logic
7) Dynamically pivot ##tblManuallyTouchedAggregate, so each load exists only once and count the number of manual touches
8) Create the highest level counts for each load
9) Update all existing records on USCTTDEV.dbo.tblRFTDetailDataHistorical where the load is in ##tblActualLoadDetailsRFT
10) Append new records into USCTTDEV.dbo.tblRFTDetailDataHistorical
11) Append new records to USCTTDEV.dbo.tblManuallyTouchedDetails
...
FROM
    ...
    LEFT JOIN nai2padm.abpp_otc_caps_analyst   abpp_otc_caps_analyst ON load_leg_r.frst_shpg_loc_cd = abpp_otc_caps_analyst.location_id
                                                                      ...
                                                                      1 )
WHERE
    ( (EXTRACT(YEAR FROM CASE WHEN load_leg_r.SHPD_DTT IS NULL THEN load_leg_r.STRD_DTT ELSE load_leg_r.SHPD_DTT END) >= EXTRACT(YEAR FROM SYSDATE)-1)
      AND ( load_leg_r.frst_ctry_cd IN (
        ''USA'',
        ''CAN'',
        ''MEX''
...
/*
Update USCTTDEV.dbo.tblRFTDetailDataHistoricalNew with data from tblRFTManuallyTouchedDetails counts

SELECT DISTINCT LoadNumber, Reason, COUNT(*) AS TouchedCount
FROM USCTTDEV.dbo.tblRFTManuallyTouchedDetails rmtd
GROUP BY LoadNumber, Reason
*/

/*
SELECT * FROM ##tblActualLoadDetailsRFT
SELECT * FROM ##tblManuallyTouchedDetails
SELECT COUNT(DISTINCT LOAD_NUMBER) AS DISTINCTCount, COUNT(*) as TotalCount
FROM ##tblActualLoadDetailsRFT
SELECT DISTINCT LOAD_NUMBER, COUNT(*) As COUNT FROM ##tblActualLoadDetailsRFT GROUP BY LOAD_NUMBER Having COUNT(*) > 1
SELECT * FROM ##tblActualLoadDetailsRFT WHERE LOAD_NUMBER = 516579486
*/
END