...
FROM
    nai2padm.tbllanes,
    nai2padm.tbllaneaudit,
    nai2padm.tblawards,
    nai2padm.tblcarriers,
    nai2padm.tbltmrates
WHERE
...
Select * into ##tblActualLoadDetailsRaw from OPENQUERY(NAJDAPRD,'SELECT
    z.*,
    ...
    (
        ...
        FROM
            (
                ...
                FROM
                    najdaadm.address_r    ad,
                    najdaadm.address_r    ad1,
                    najdaadm.load_at_r    la,
                    najdaadm.load_leg_r   l,
                    najdaadm.carrier_r    c,
                    najdaadm.status_r     s
                WHERE
                    l.frst_shpg_loc_cd = la.shpg_loc_cd
                    AND l.cur_optlstat_id = s.stat_id
                    AND l.frst_addr_id = ad.addr_id
                    AND l.carr_cd = c.carr_cd
        ...
        FROM
            (
                ...
                FROM
                    ...
                    najdaadm.shipment_r          s,
                    najdaadm.load_leg_r          l
                WHERE
                    ld.ld_leg_id = l.ld_leg_id
                    AND TO_CHAR(
                        CASE
                            WHEN l.strd_dtt IS NULL THEN
                                l.shpd_dtt
...
/*
Adjust linehaul costs by @fuelDifferential variable value
*/
DECLARE @fuelDifferential AS DECIMAL(5,2)
...
/*
Adjust fuel costs by @fuelDifferential variable value
*/
DECLARE @fuelDifferential AS DECIMAL(5,2)

SET @fuelDifferential = .032
UPDATE ##tblActualLoadDetailsALD
SET Act_Linehaul = CASE when EQMT_TYP not in ('53IM') THEN 0.32*FIXD_ITNR_DIST
	else 
(0.32*FIXD_ITNR_DIST)/2 END
WHERE SRVC_CD='OPEN'
AND CAST(Act_Fuel as NVARCHAR(10)) in (' ','','.','-')
        ...
        FROM
            (
                ...
                WHERE
                    ...
                    AND l.cur_optlstat_id >= 320
                    AND ( EXTRACT(YEAR FROM strd_dtt) >= EXTRACT(YEAR FROM SYSDATE)
                          OR strd_dtt IS NULL )
                    AND EXTRACT(YEAR FROM l.shpd_dtt) >= EXTRACT(YEAR FROM SYSDATE)
                    AND l.eqmt_typ IN (
                        ''53FT'',
                        ''53IM'',
                        ''53RT'',
