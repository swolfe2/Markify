USE [USCTTDEV]
GO
/****** Object:  StoredProcedure [dbo].[sp_HistoricRateAwards]    Script Date: 10/21/2019 4:30:37 PM ******/
SET ANSI_NULLS ON
GO
...
(SELECT
  ...
  bar.bid_rpm,
  bar.award_pct,
  (SUM(bar.award_pct) OVER (PARTITION BY bal.orig_city_state + '-' + bal.dest_city_state)) AS AwardSum,
  cnt.WeightedRPM,
...
GROUP BY lane) AS cnt
  ON cnt.lane = bar.Lane
) AS DATA
