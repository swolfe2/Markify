SELECT DISTINCT CAST(data.load_Number as INT) AS LOAD_NUMBER,
data.SHIPMENT_NUMBER,
--data.APPOINTMENT_FROM_TIME,
LEFT(data.SHIPMENT_NUMBER,8) AS PO,
CAST(FORMAT(data.originalappointment, 'MM/dd/yyyy') AS DATETIME) AS 'Original Appointment',
--CAST(FORMAT(data.originalappointmentadded, 'MM/dd/yyyy') AS DATETIME) AS ORIGINAL_APPOINTMENT_ADDED,
--CAST(FORMAT(data.APPOINTMENT_FROM_TIME, 'MM/dd/yyyy HH:mm:ss') AS DATETIME) AS APPOINTMENT_FROM_TIME,
CAST(FORMAT(data.APPOINTMENT_FROM_TIME, 'MM/dd/yyyy') AS DATETIME) AS 'New Delivery Date',
DATEDIFF(day,CAST(FORMAT(data.originalappointment, 'MM/dd/yyyy') AS DATETIME),CAST(FORMAT(data.APPOINTMENT_FROM_TIME, 'MM/dd/yyyy') AS DATETIME)) as 'Days Diff',
--data.APPOINTMENT_CHANGE_TIME,
CAST(FORMAT(data.APPOINTMENT_CHANGE_TIME, 'MM/dd/yyyy HH:mm:ss') AS DATETIME) AS APPOINTMENT_CHANGE_TIME,
--data.APPOINTMENT_CHANGED_BY,
--data.NAME,
data.REASON_CODE,
data.REASON_DESC,
CAST(data.stop_number AS INT) AS STOP_NUMBER,
data.LOAD_STATUS,
loc.FRST_SHPG_LOC_CD,
loc.FRST_SHPG_LOC_NAME,
loc.OrderType
        ...
        FROM
            ...
            INNER JOIN (
                ...
                FROM
                    nai2padm.abpp_otc_appointmenthistory aph
                WHERE
                    appointment_change_time >= SYSDATE - 5
                    AND reason_code <> ''APC_''
                    AND stop_number > 1
                GROUP BY
                    aph.load_number,
...
ORDER BY
    ...
    aph.stop_number ASC
')data
INNER JOIN (SELECT DISTINCT LD_LEG_ID, FRST_SHPG_LOC_CD, FRST_SHPG_LOC_NAME, OrderType
FROM USCTTDEV.dbo.tblActualLoadDetail) loc on loc.LD_LEG_ID = data.load_number
WHERE DATEDIFF(day,CAST(FORMAT(data.originalappointment, 'MM/dd/yyyy') AS DATETIME),CAST(FORMAT(data.APPOINTMENT_FROM_TIME, 'MM/dd/yyyy') AS DATETIME)) <> 0
ORDER BY load_number ASC, shipment_number ASC, appointment_change_time DESC