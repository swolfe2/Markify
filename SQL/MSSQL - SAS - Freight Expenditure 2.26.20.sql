USE [USCTTDEV]
GO
/****** Object:  StoredProcedure [dbo].[sp_ActualLoadDetail]    Script Date: 2/26/2020 8:56:24 AM ******/
SET ANSI_NULLS ON
GO
...
/*
-- =============================================
-- Author:		<Steve Wolfe, steve.wolfe@kcc.com, Central Tlaransportation Team>
...
-- 2/25/2020 - SW - Changed spacemaker logic to MAX(CASE WHEN s.rfrc_num6 IN (''ZUSM'',''ZNSM'') THEN ''Y'' END) AS Spacemaker
...
Added some logic to add SPACEMAKERS to dataset. TODO: Add data to SQL table once Lynlee validates.
...
WHERE CASE WHEN ald.SHPD_DTT IS NULL THEN ald.STRD_DTT ELSE ald.SHPD_DTT END BETWEEN lane.EffectiveDate and lane.ExpirationDate
...
WHERE z.ZN_DESC LIKE ''%,%''
AND NOT(REGEXP_LIKE(SUBSTR(z.ZN_CD,1,1), ''^[[:digit:]]+$''))
AND z.ZN_CD NOT LIKE ''%-%%''
...
SELECT * FROM USCTTDEV.dbo.tblTMSZones WHERE LastUpdated <> '2020-02-25 13:41:15.573'
