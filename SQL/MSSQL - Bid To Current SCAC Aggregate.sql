...
*/
CAST(SUM(regionAgg.LY_LANE_VOL) AS INT) AS LY_LANE_VOL,
CAST(SUM(regionAgg.LY_VOL) AS INT) AS LY_VOL,
CAST(ROUND(SUM(regionAgg.LY_VOL) / SUM(CAST(LY_LANE_VOL AS NUMERIC(10,2))), 4) AS NUMERIC(10,4)) AS LY_AWARD_PCT,
CAST(SUM(regionAgg.LY_COST) AS NUMERIC(10,2)) AS LY_COST,

/*
This is an aggregation of the current bid
*/
CAST(SUM(regionAgg.TY_LANE_BID_VOL) AS INT) AS TY_LANE_BID_VOL,
CAST(SUM(regionAgg.TY_BID_VOL) AS INT) AS TY_BID_VOL,
CAST(COALESCE(SUM(regionAgg.TY_BID_VOL),0) - COALESCE(SUM(regionAgg.LY_VOL),0)  AS INT) AS TY_BID_VOL_GAIN_LOSS,
CAST(ROUND(SUM(regionAgg.TY_BID_VOL) / SUM(CAST(TY_LANE_BID_VOL AS NUMERIC(10,2))), 4) AS NUMERIC(10,4)) AS TY_BID_AWARD_PCT,
CAST(SUM(regionAgg.TY_BID_COST) AS NUMERIC(10,2)) AS TY_BID_COST,

/*
This is an aggregation of the most current state
*/
CAST(SUM(regionAgg.CUR_LANE_VOL) AS INT) AS CUR_LANE_VOL,
CAST(SUM(regionAgg.CUR_AWARD_VOL) AS INT) AS CUR_AWARD_VOL,
CAST(COALESCE(SUM(regionAgg.CUR_AWARD_VOL),0) - COALESCE(SUM(regionAgg.TY_BID_VOL),0) AS INT) AS CUR_AWARD_VOL_GAIN_LOSS,
CAST(ROUND(SUM(regionAgg.CUR_AWARD_VOL) / SUM(CAST(CUR_LANE_VOL AS NUMERIC(10,2))), 4) AS NUMERIC(10,4)) AS CUR_AWARD_PCT,
CAST(SUM(regionAgg.CUR_COST) AS NUMERIC(10,2)) AS CUR_COST,
CAST(SUM(regionAgg.CUR_V_BID_COST) AS NUMERIC(10,2)) AS CUR_V_BID_COST
...
GROUP BY regionAgg.CARR_CD,
regionAgg.Name,
regionAgg.SCAC,
regionAgg.SRVC_DESC,
regionAgg.Broker

ORDER BY regionAgg.CARR_CD ASC, regionAgg.SCAC ASC