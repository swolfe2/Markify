...
*/
DECLARE @loads TABLE(RowNumber INT, LoadNumber VARCHAR(20), LoadNumberString VARCHAR(30))
INSERT INTO @loads
	SELECT ROW_NUMBER() OVER (ORDER BY loads.LD_LEG_ID ASC) AS RowNum,
	loads.LD_LEG_ID,
	loads.LD_LEG_ID_STRING FROM (
    SELECT DISTINCT ald.LD_LEG_ID,'''''' + CAST(ald.LD_LEG_ID AS VARCHAR(20)) + '''''' AS LD_LEG_ID_STRING 
...
SET @loadGroupings = (SELECT CEILING(COUNT(*) / CAST (500 AS NUMERIC(10,2))) FROM @loads)
SET @i = 1

/*
Create a recordset with unique group numbers for each 500 records to process since the is a limit from Snowflake
*/
DECLARE @loadGroups TABLE(RowNumber INT, LoadNumber VARCHAR(20), LoadNumberString VARCHAR(30), GroupNumber INT)
...
*/
DECLARE @loadNumberStrings TABLE (GroupNumber INT, LoadCount INT, LoadArray NVARCHAR(MAX))
...
WHILE @i <= @loadGroupings
	BEGIN
		...
		*/
		SET @loadCount = (SELECT COUNT(*) FROM @loadGroups WHERE GroupNumber = @i)
		...
		*/
		INSERT INTO @loadNumberStrings SELECT @i, @loadCount, @loadNumbers
...
/*
SET @queryString = '''SELECT TOP 1 * FROM CONTROL_TOWER.INT.SHIPMENT_HUB WHERE SHIPMENT_NUM IN (' + @loadNumbers + ')'')'
*/
...
WHILE @i <= @loadGroupings
	...
	*/
	SET @LoadNumbers = (SELECT LoadArray FROM @loadNumberStrings WHERE GroupNumber = @i)
		...
		Insert into @loadNumberStrings CTE for faster processing through SQL, since it will have a limited number of rows to process
		*/
		INSERT INTO @fourKitesData EXEC(@openquery + @querystring)

		SET @i =  @i + 1
	END

SELECT * FROM @fourKitesData
