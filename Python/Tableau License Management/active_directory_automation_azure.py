...
Process was developed using Python Version 3.10.8

Last update: 1/7/2025 - Conversion of MSSQL to Azure

Process steps overview:
1. Create dataframe from flat file
2. Push dataframe to Azure server
3. Execute Azure stored procedure
...
def push_to_azure(df, conn):
    ...
    azure_database.execute_stored_procedure(conn, "dbo.sp_ActiveDirectoryAutomation")
...
def create_dataframe():
    ...
    if time_difference_days > max_days:
        error_message = f"""It has been {time_difference_days} days since the {full_file_path}
             ...
             Please ensure the file is processing correctly."""

        to_addresses = "jayagopal.ashok@kcc.com; mario.sarmiento@kcc.com; steve.wolfe@kcc.com; tatch@kcc.com"
    ...
    if file_size < 1000:
        ...
        query = f"""
        INSERT INTO [{AZURE_DATABASE}].dbo.tblSentEmails(SentOn, EmailType, ToAddresses, Subject, Message)
        SELECT GETDATE(), '{process_step}', '{str(to_addresses).replace("'", "''")}', 
        'Tableau License Automation Failure: {process_step}', '{error_message}'
        """
...
def main():
    ...
    # Appends dataframe to Azure Server
    print("Pushing data to Azure")
