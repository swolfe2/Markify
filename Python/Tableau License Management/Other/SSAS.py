...
def _parse_DAX_result(table: "DataTable") -> pd.DataFrame:
    ...
    col_types = {
        c.ColumnName: types_map.get(c.DataType.FullName, "object") for c in cols
    }
    ...
    col_types.update(
        {k: float for k in set(ser[ser].index).intersection(col_types_ints)}
    )
...
@_assert_dotnet_loaded
def process_model(
    connection_string, db_name, refresh_type="full", item_type="model", item=None
):
    """
    ...
    if item_type.lower() == "table" and not item:
        raise ValueError(
            "If item_type is table, must supply an item (a table name) to process"
        )
