...
def main():
    ...
    data_dict = mssql_database.execute_query_to_dictonary(
        conn,
        "SELECT * FROM TableauLicenses.dbo.v_RegisteredInactiveKCEmployee WHERE EmailSentOn IS NULL",
    )
    ...
    if len(data_dict) > 0:
        ...
        for index in range(len(data_dict)):
            ...
            if assigned_email_manager == registered_email_manager:
                to = assigned_email_manager
            else:
                to_list = [
                    assigned_email_manager,
                    registered_email_manager,
            ...
            if assigned_email == registered_email:
                ...
                cc_list = [assigned_email, registered_email]
