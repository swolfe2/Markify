...
def main():
    ...
    data_dict = mssql_database.execute_query_to_dictonary(
        conn,
        "SELECT * FROM TableauLicenses.dbo.v_LastUsedByNonKCEmail WHERE EmailSentOn IS NULL",
    )
    ...
    if len(data_dict) > 0:
        ...
        for index in range(len(data_dict)):
            ...
            registered_email = data_dict[index]["LastRegisteredEmail"].lower()
            ...
            html_greeting = f"<p>Hello {last_registered_user_name},</p>"
            ...
            html_body = f"""
                ...
                <body>
                {html_greeting}
                <p>During a current audit on Tableau licenses, it was discovered that the license below has last been used by a 
                person without a Kimberly-Clark email address which is a violation of the Tableau End User License Agreement (EULA).</p>
                ...
                <p>In order to resolve the violation, <span style='background-color: #FFFF00'><b>please reply all to this email by EOB {friday}</b></span>
                 with information on who in Kimberly-Clark provided you with this license number. If no response is received by 
                {friday}, then we will escalate this issue.</p> 
                <p>Please ensure that you are following the license deactivation process 
                available from <a href='https://help.tableau.com/current/desktopdeploy/en-us/desktop_deploy_move_or_deactivate.htm'>Tableau</a>
                 to deactivate this license, and reply back with a screenshot showing the completion of the action.</p>
