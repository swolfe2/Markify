...
from app.ds_config import DS_CONFIG, DS_JWT, EXAMPLES_API_TYPE
    ...
    def _jwt_auth(cls):
        ...
        try:
            ...
            if "consent_required" in body:
                ...
                consent_url = (
                    f"{DS_CONFIG['authorization_server']}/oauth/auth?response_type=code&"
    ...
    def login(cls, auth_type):
        if auth_type == "code_grant":
            return cls.get(auth_type).authorize(
                callback=url_for("ds.ds_callback", _external=True)
            )
        elif auth_type == "jwt":
