...
# Spreadsheet ID
spreadsheetId = '1bjBg7BOklOd4dXa2FkHpnEkJ_PGkztT1pKLBZfExFAU'
"""
# Data range to create dataframe, and create raw file document
getDataRangeName = 'ExportForDownload!A:P'
...
df.to_excel('\\\\10.29.7.46\\c$\\Users\\wolfes\\Desktop\\Innowera Files - DO NOT DELETE\\Doc Cancel\\tblDocCancelRaw.xlsx',
...
df = DataFrame(data=values)
dfPathName = ""
df.to_excel('\\\\10.29.7.46\\c$\\Users\\wolfes\\Desktop\\Innowera Files - DO NOT DELETE\\Doc Cancel\\Google Sheet Archive\\Google Sheet - '+currentDateTime+'.xlsx',
            header=False, index=False)
...
SQLRangeName = 'ExportForDownload!A2:P'
...
sql = 'select * from tblInnoweraDocCancelHistorical Where ([InboundEmailSentOn] >= GetDate()-90) ORDER BY [ID]'
cursor = cnxn.cursor()
cursor.execute(sql)
...
"""
df['InboundEmailSentOn'] = df['InboundEmailSentOn'].dt.strftime(
    '%m/%d/%Y %H:%M:%S')
df['AppsScriptProcessedOn'] = df['AppsScriptProcessedOn'].dt.strftime(
    '%m/%d/%Y %H:%M:%S')
df['OutboundEmailsSent'] = df['OutboundEmailsSent'].dt.strftime(
    '%m/%d/%Y %H:%M:%S')
...
# Google Sheet tab range to clear/reappend
SQLRangeName2 = 'dbo_tblInnoweraDocCancelHistorical!A:W'
...
# Building Service to Clear Google Sheet
request = service.spreadsheets().values().clear(spreadsheetId=spreadsheetId,
                                                range=SQLRangeName2, body=clear_values_request_body)
response = request.execute()

# Prints response that Google Sheet has been cleared
responseText = '\n'.join(
    [str(response), 'The Historical Google Sheet has been cleared!'])
print(responseText)
...
request = service.spreadsheets().values().append(spreadsheetId=spreadsheetId, range=SQLRangeName2,
