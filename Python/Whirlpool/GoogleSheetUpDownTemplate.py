...
# Pandas reading values from Excel File, and building table. Change sheet_name to whatever your sheet is called
excelData = pandas.read_excel(
    "C:\\Users\\wolfes\\Desktop\\Python Files\\tblActiveEmployees.xlsx", sheet_name="Sheet1")

# Pandas building dataframe, and exporting .xlsx copy of table
df = DataFrame(data=excelData)

"""
# Google Sheets API can't handle date/time. Below converts certain headers to formatted text strings. Here are examples of mm/dd/yyyy and date/time
df['Date'] = df['Date'].dt.strftime('%m/%d/%Y')
df['DateTime'] = df['DateTime'].dt.strftime('%m/%d/%Y %H:%M:%S')
"""

# Converting the data set to different arrays
dfHeaders = df.columns.values.tolist()
dfHeadersArray = [dfHeaders]

# Google Sheets can't parse NaN values, have to replace them with blank strings
dfDataClean = df.fillna('')

dfData = dfDataClean.values.tolist()

"""Writing to Google Sheet Range - Uncomment lines below if you want to see the .csv values in the terminal"""
#print(dfHeaders)
#print(dfData)
