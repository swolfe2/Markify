...
# Spreadsheet ID
spreadsheetId = '1nIpT0WltH6AlKkpwyXWP_eIUvj6nvUSQU1dheS-0kEw'

# Data range to create dataframe, and create raw file document
getDataRangeName = 'dbo_tblAIDMaster'
"""
...
df.to_excel('\\\\10.29.6.136\\c$\\Users\\wolfes\\Desktop\\Innowera Files - DO NOT DELETE\\OMAR - Model Home\\tblAIDMaster.xlsx',
...
sql = 'select * from tblAIDMaster ORDER BY [ID]'
cursor = cnxn.cursor()
cursor.execute(sql)
...
df['DateAdded'] = df['DateAdded'].dt.strftime('%m/%d/%Y')
df['LastSale'] = df['LastSale'].dt.strftime('%m/%d/%Y')
df['LastUpdateDate'] = df['LastUpdateDate'].dt.strftime('%m/%d/%Y')
df['TableLastUpdate'] = df['TableLastUpdate'].dt.strftime('%m/%d/%Y')


df.to_excel('\\\\10.29.6.136\\c$\\Users\\wolfes\\Desktop\\Python Files\\tblAIDMaster.xlsx',
...
# Prints completion
print("Whirlpool Accounts Completed!")
