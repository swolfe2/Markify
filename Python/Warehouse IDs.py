...
1) Get specific file from File Directory, quit process if file was last modified more than 36 hours ago
...
def cleanMSSQL():
    for col in WarehouseIDs.columns:
        ...
        sqlCleanString = ("UPDATE ##tblWarehouseIDs SET [" + col + "] = NULL WHERE [" + col + "] IN ('0', '0.0', 'nan')")
        # cleans the previous head insert
        with tembdb_connection.cursor() as cursor:
...
WarehouseIDs.head().to_sql('##tblWarehouseIDs', 
                    con=tempdb_engine, 
                    if_exists = 'append', 
                    index = False, 
                    dtype = outputdict)
