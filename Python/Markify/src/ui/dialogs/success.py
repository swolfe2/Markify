...
def show_success_dialog(
    ...
    def open_folder(event=None):
        if single_mode:
            subprocess.Popen(f'explorer /select,"{abs_path}"', shell=True)  # nosec B602 B607 - Safe: user-selected file path
        else:
            subprocess.Popen(f'explorer "{abs_path}"', shell=True)  # nosec B602 B607 - Safe: user-selected folder path
